...
def create_access_token(account_id: Optional[str] = None):
    """This method creates an access token"""
    nb_days = settings.JWT_ACCESS_TOKEN_EXPIRATION 
    future_exp_time = datetime.now(timezone.utc) + timedelta(days=nb_days)
    registred_claims = {
        "iss": "WindForLife",
        "sub": "WindForLife_api",
...
def create_refresh_token(account_id: str):
    ...
    nb_days: int = settings.JWT_REFRESH_TOKEN_EXPIRATION
    future_exp_time: datetime = datetime.now(timezone.utc) + timedelta(days=nb_days)
